...
>(({ className, children, side, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed right-0 top-0 z-50 h-full w-full border-l bg-[var(--bg-card)] p-6 sm:max-w-md lg:max-w-lg',
        className,
      )}
      {...props}
    >
      {children}
      <SheetClose className="absolute right-4 top-4 text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
        <X className="h-4 w-4" />
